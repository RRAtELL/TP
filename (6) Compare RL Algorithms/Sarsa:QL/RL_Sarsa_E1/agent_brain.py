...
class SarsaTable:
    ...
    def error(self, state, action, reward, next_state, next_action):
    ...
    def plot_results(self, steps, cost, error):
        #
    ...
    def cumreward_normalized(self, rewards_cache):

        cum_rewards = []
        rewards_mean = np.array(rewards_cache).mean()
        rewards_std = np.array(rewards_cache).std()
        count = 0  # used to determine the batches
        cur_reward = 0  # accumulate reward for the batch
        for cache in rewards_cache:
            count += 1
            cur_reward += cache
            if(count == 20):
        ...
        plt.plot(cum_rewards, label="SARSA")
        plt.ylabel('Cumulative Rewards')
        plt.xlabel('Batches of Episodes (sample size 10) ')
        plt.title("Convergence of Cumulative Reward")
